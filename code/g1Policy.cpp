...
#include "gc/g1/g1AdaptivePolicy.hpp"

G1Policy::G1Policy(STWGCTimer* gc_timer) :
  _g1h(G1CollectedHeap::heap()),
  _predictor(G1ConfidencePercent / 100.0),
  _analytics(new G1Analytics(&_predictor)),
  _adaptive_policy(new G1AdaptivePolicy(_g1h, _analytics)),  // 添加逗号
  _remset_tracker(),
...
double G1Policy::mixed_gc_live_threshold() const {
  return _adaptive_policy->mixed_gc_live_threshold();
}

uint G1Policy::mixed_gc_count_target() const {
  return _adaptive_policy->mixed_gc_count_target();
}

double G1Policy::old_cset_region_threshold() const {
  return _adaptive_policy->old_cset_region_threshold();
}

double G1Policy::heap_waste_percent() const {
  return _adaptive_policy->heap_waste_percent();
}
...
void G1Policy::record_collection_pause_end(double pause_time_ms, bool concurrent_operation_is_full_mark) {
  ...
  _adaptive_policy->update_after_gc(*phase_times());
}
